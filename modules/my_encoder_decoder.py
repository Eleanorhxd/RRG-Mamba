...
from modules.Bi_direct_adapter import Bi_direct_adapter
from modules.ContraNorm import ContraNorm
# from modules.region_attention import CrossRegionAttntion
def clones(module, N):
...
class Decoder(nn.Module):
    def __init__(self, layer, N):
        ...
        self.layers = clones(layer, N)
        self.norm = LayerNorm(layer.d_model)
        self.bi_adapter = Bi_direct_adapter(dim=8) #  加双向适配器

        self.contraNorm = ContraNorm(dim=512, scale=0.1, dual_norm=False, pre_norm=False, temp=0.07, learnable=False, positive=False, identity=False)
...
class RelationalMemory(nn.Module):
    ...
    def forward_step(self, input, memory):
        ...
        # print('next.shape',next_memory.shape)
...
class EncoderDecoder(AttModel):
    ...
    def __init__(self, args, tokenizer):
        ...
        self.fbl = args.fbl
        self.clip = args.clip
        # self.cga = CrossRegionAttntion(dim=self.d_model, num_heads=self.num_heads, region_size=8)
    ...
    def _prepare_feature_forward(self, att_feats, att_masks=None, seq=None):
        ...
        if seq is not None:
            ...
            seq_mask = None
        # print('feats:', att_feats.shape) #（32，99，512）
        # print('masks:', att_masks.shape) #（[32, 1, 99])）
