...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        with tqdm(desc='Epoch %d - test' % epoch, unit='it', total=len(self.test_dataloader)) as pbar:
            with torch.no_grad():
                ...
                self.writer.add_scalar('data/test_Cider', test_met['Cider'], epoch)
    ...
    def _test(self):
        ...
        with tqdm(desc='Epoch %d - test' % epoch, unit='it', total=len(self.test_dataloader)) as pbar:
            with torch.no_grad():
                ...
                save_dir1 = r'/root/reports'
                os.makedirs(save_dir1, exist_ok=True)
        # 记录生成报告和图像ID到CSV文件
                gen_df = pd.DataFrame({'image_id': img_ids, 'generated_report': test_res})
                gen_df.to_csv(os.path.join(save_dir1, 'generate.csv'), index=False)
    
    # 记录真实报告和图像ID到CSV文件
                gt_df = pd.DataFrame({'image_id': img_ids, 'ground_truth_report': test_gts})
                gt_df.to_csv(os.path.join(save_dir1, 'ground_truth.csv'), index=False)
    ...
    def plot(self):
        ...
        os.makedirs(os.path.join('/root/atten', "attentions"), exist_ok=True)
        os.makedirs(os.path.join('/root/atten', "attentions_entities"), exist_ok=True)
